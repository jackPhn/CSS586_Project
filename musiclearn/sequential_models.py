...
def simplified_wavenet(input_shape, n_vocab):
    ...
    model.add(
        Conv1D(
            64, 3, padding="causal", activation="relu", input_shape=(input_shape[1], input_shape[2])
        )
    )
...
def train_model(model, sequence_length, model_name, epochs=20, batch_size=128):
    ...
    checkpoint_cb = ModelCheckpoint(
        filepath, monitor="loss", verbose=0, save_best_only=True, node="min"
    )

    log_dir = str(logs_dir / "logs" / model_name)
    tensorboard_cb = TensorBoard(
        log_dir=log_dir, write_graph=True, write_images=True, update_freq="epoch"
    )
